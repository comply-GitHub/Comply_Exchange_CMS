...
function TinTable({ setOpen2, setId1, getAgentTinData }) {
  ...
  useEffect(() => {
    if (params?.id) dispatch(getAgentTinTypeById(params?.id));
  }, [params]);
  ...
  const handleChange = async (e, item) => {
    ...
    let selectedRow = tempAgentTinData?.filter(
      (x) => x.taxpayerIdTypeID === item.taxpayerIdTypeID
    )[0];
          ...
          {getAgentTinData?.AgentTinTypeId?.map((item) => (
            <TableRow key={item.id}>
              ...
              <TableCell>
                <div>
                  <Checkbox
                    name="nonUSIndividual"
                    checked={item.nonUSIndividual}
                    value={item.nonUSIndividual}
                    onChange={(e) => {
                      handleChange(e, item);
                    }}
                  />
              ...
              <TableCell>
                <div className="d-flex">
                  <Checkbox
                    name="usIndividual"
                    checked={item.usIndividual}
                    value={item.usIndividual}
                    onChange={(e) => {
                      handleChange(e, item);
                    }}
                  />
              ...
              <TableCell>
                <div className="d-flex">
                  <Checkbox
                    name="nonUSEntity"
                    checked={item.nonUSEntity}
                    value={item.nonUSEntity}
                    onChange={(e) => {
                      handleChange(e, item);
                    }}
                  />
              ...
              <TableCell>
                <div className="d-flex">
                  <Checkbox
                    name="nonUSGovernment"
                    checked={item.nonUSGovernment}
                    value={item.nonUSGovernment}
                    onChange={(e) => {
                      handleChange(e, item);
                    }}
                  />
